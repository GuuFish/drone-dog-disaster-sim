...
export const platformDeviceCode=d=>`SIM-${d.type==='uav'?'UAV':'ROBOT'}-${String((d.typeBerth??d.berth??0)+1).padStart(3,'0')}`;
...
export function normalizeBerths(state){
 ...
 n.sessionId??=uid();n.platformMode??=false;n.sequence??={};n.processedCommands??=[];n.eventSeq??=0;
...
export function fresh(){const s={version:4,time:0,auto:false,platformMode:false,sessionId:uid(),sequence:{},processedCommands:[],eventSeq:0,devices:[],events:[],tasks:[],logs:[],messages:[]};for(const [i,type] of ['uav','uav','dog','dog'].entries())s.devices.push(createDevice(s,{type,name:(type==='uav'?'苍穹 ':'猎隼 ')+(i%2+1).toString().padStart(2,'0')}));log(s,'集结中心已就绪 · 2 架无人机 / 2 台机器狗');return s;}
...
export function emit(s,type,deviceId,payload){const d=s.devices.find(x=>x.id===deviceId),sequence=(s.sequence[deviceId]||0)+1;s.sequence[deviceId]=sequence;const body={deviceCode:d&&platformDeviceCode(d),...payload};s.messages=[{schemaVersion:'1.0',messageId:uid(),sessionId:s.sessionId||'local',type,source:'simulator',deviceId,correlationId:body.taskCode||body.taskId||body.eventId||null,sequence,timestamp:new Date().toISOString(),simulationTimeSeconds:s.time,payload:body},...(s.messages||[])].slice(0,150);}
// 事件编号短可读（person-01 / fire-02…），平台目标编号为 SIM-<事件编号>
export function addEvent(s,kind,x,z,severity){const n=structuredClone(s);let seq=(n.eventSeq||0)+1,id=`${kind}-${String(seq).padStart(2,'0')}`;while(n.events.some(e=>e.id===id)){seq++;id=`${kind}-${String(seq).padStart(2,'0')}`;}n.eventSeq=seq;n.events.push({id,kind,x,z,severity,status:'待侦察',created:s.time});log(n,`已布设${TYPES[kind]} · 尚未被设备发现`);return n;}
...
export function task(s,d,phase,route,eventId,meta={}){const length=routeLength(d,route),t={id:meta.taskId||uid(),platformTaskId:meta.taskId||null,platformTaskCode:meta.taskCode||null,commandId:meta.commandId||uid(),eventId,deviceId:d.id,deviceName:d.name,phase,status:'执行中',stage:'前往目标',route,index:0,hold:0,progress:0,lastReportedProgress:-1,totalDistance:length,travelled:0,etaSeconds:length/(d.type==='uav'?WORLD.uavSpeed:WORLD.dogSpeed)+(phase==='返回基地'?0:20),started:s.time};s.tasks.unshift(t);d.state=phase;emit(s,'task.ack',d.id,{commandId:t.commandId,taskId:t.platformTaskId||t.id,taskCode:t.platformTaskCode,accepted:true});return t;}
...
// 平台联动模式下无人机仍执行自主巡航搜索并上报发现，机器狗派遣仍由平台统一决定（见 tick 中的 !platformMode 分支）
export function startDevices(s,deviceId){const n=structuredClone(s);n.auto=true;for(const d of n.devices.filter(d=>!deviceId||d.id===deviceId)){if(d.battery<=15){log(n,`${d.name} 电量不足，无法启动`);continue;}d.enabled=true;d.returning=false;d.recharging=false;d.patrol=0;d.patrolKey=null;if(!busy(n,d.id))d.state=n.platformMode?'平台待命':d.type==='uav'?'搜索中':'等待复核任务';}log(n,deviceId?'指定设备已启动':'全部可用设备已启动 · '+(n.platformMode?'等待平台指令':'机器狗等待复核目标'));return n;}
export function cancelTask(n,t){t.status='已取消';t.stage='已取消';const d=n.devices.find(d=>d.id===t.deviceId);if(d)d.state='待命';const e=n.events.find(e=>e.id===t.eventId);if(e)e.status=t.phase==='空中侦察'?'待侦察':'待复核';emit(n,'task.result',t.deviceId,{taskId:t.platformTaskId||t.id,taskCode:t.platformTaskCode,outcome:'CANCELLED'});log(n,`${t.deviceName} 的任务已取消`);}
...
function finish(n,t,d,e){t.status='已完成';t.progress=100;t.finished=n.time;t.stage=t.phase==='返回基地'?'已归位':'结果已上报';d.state=t.phase==='返回基地'?'待命':d.type==='dog'?'原地待命':'任务完成';if(e){e.status=d.type==='uav'?'待复核':n.platformMode?'待确认':'已处理';e.discoveredBy=e.discoveredBy||d.name;e.result=d.type==='uav'?'空中目标已确认，等待地面复核':n.platformMode?'现场复核完成：模拟采样与风险标记已上报':'现场处理完成：模拟采样与风险标记已上报';if(d.type==='dog'){e.processedAt=n.time;e.processedWallTime=Date.now();}emit(n,d.type==='uav'?'event.discovered':'task.result',d.id,{taskId:t.platformTaskId||t.id,taskCode:t.platformTaskCode,eventId:e.id,outcome:'SUCCEEDED',observations:e.result,position:{x:e.x,z:e.z}});}else emit(n,'task.result',d.id,{taskId:t.platformTaskId||t.id,taskCode:t.platformTaskCode,outcome:'SUCCEEDED'});log(n,`${d.name} · ${t.phase==='返回基地'?'已归位':e?.result||'任务完成'}`);}
function tick(s,dt){let n=structuredClone(s);n.time+=dt;for(const t of n.tasks.filter(t=>t.status==='执行中')){const d=n.devices.find(d=>d.id===t.deviceId),e=n.events.find(e=>e.id===t.eventId);if(!d)continue;if(d.battery<=0){t.status='失败';t.stage='电量耗尽';d.state='电量耗尽';if(e)e.status=d.type==='uav'?'待侦察':'待复核';emit(n,'task.result',d.id,{taskId:t.platformTaskId||t.id,taskCode:t.platformTaskCode,outcome:'FAILED',reason:'LOW_BATTERY'});log(n,`${d.name} 电量耗尽，任务中止`);continue;}
  if(t.index<t.route.length){const ready=d.type!=='uav'||d.y>=surfaceHeight(d.x,d.z)+20;flightHeight(d,dt);t.stage=ready?'前往目标':'起飞中';if(ready){const m=move(d,t.route,t.index,(d.type==='uav'?WORLD.uavSpeed:WORLD.dogSpeed)*dt);t.index=m.index;t.travelled+=m.travelled;}d.y=Math.max(d.y,surfaceHeight(d.x,d.z)+(d.type==='uav'?20:parkedHeight(d.type)));if(d.type==='dog')flightHeight(d,dt);t.progress=Math.min(80,t.travelled/Math.max(1,t.totalDistance)*80);if(n.platformMode&&d.type==='uav'){for(const event of n.events)if(event.status==='待侦察'&&distance(event,d)<=WORLD.searchRadius){event.status='待复核';event.discoveredBy=d.name;emit(n,'event.discovered',d.id,{eventId:event.id,eventType:event.kind,position:{x:event.x,z:event.z},severity:event.severity});log(n,`${d.name} 搜索范围内发现${TYPES[event.kind]}`);}}
  ...
  d.battery=Math.max(0,d.battery-dt*(d.type==='uav'?.012:.003));t.etaSeconds=Math.max(0,(t.totalDistance-t.travelled)/(d.type==='uav'?WORLD.uavSpeed:WORLD.dogSpeed)+(t.phase==='返回基地'?0:20-t.hold));if(n.platformMode&&t.platformTaskCode&&Math.floor(t.progress/10)>Math.floor((t.lastReportedProgress??-1)/10)){t.lastReportedProgress=t.progress;emit(n,'task.progress',d.id,{taskId:t.platformTaskId,taskCode:t.platformTaskCode,progress:Math.floor(t.progress),phase:t.stage,position:{x:d.x,y:d.y,z:d.z}});}}
 if(n.auto){
 ...
 for(const d of n.devices)if(d.type==='dog'&&!busy(n,d.id)&&d.battery<=15&&d.enabled){d.enabled=false;d.state='低电量待命';log(n,`${d.name} 电量不足，已停止自主运行`);}
  if(!n.platformMode&&Math.floor(n.time/5)>Math.floor(s.time/5))for(const d of n.devices)emit(n,'device.telemetry',d.id,{position:{x:d.x,y:d.y,z:d.z},state:d.state,battery:d.battery});return n;}
